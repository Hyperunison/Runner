    ...
    def build(self, ucdm: List[Dict[str, UCDMConvertedField]], person_id_generator: PersonIdGenerator):
        ...
        with open(filename, 'w', newline='') as file:
            ...
            for row in ucdm:
                ...
                output["location_id"] = self.render_ucdm_value(row, 'location_id')
                output["provider_id"] = self.render_ucdm_value(row, 'provider_id')
                output["care_site_id"] = self.render_ucdm_value(row, 'care_site_id')
                ...
                output["gender_source_concept_id"] = self.render_omop_id(row, 'gender_source')
                output["race_source_value"] = self.render_ucdm_value(row, 'race')
                output["race_source_concept_id"] = self.render_omop_id(row, 'race_source')
                output["ethnicity_source_value"] = self.render_ucdm_value(row, 'ethnicity')
                output["ethnicity_source_concept_id"] = self.render_omop_id(row, 'ethnicity_source')
