    ...
    def build(self, ucdm: List[Dict[str, UCDMConvertedField]], person_id_generator: PersonIdGenerator):
        ...
        with open(filename, 'w', newline='') as file:
            ...
            for row in ucdm:
                ...
                output["location_id"] = self.render_omop_id(row, 'c.location_id')
                output["provider_id"] = self.render_omop_id(row, 'c.provider_id')
                output["care_site_id"] = self.render_omop_id(row, 'c.care_site_id')
