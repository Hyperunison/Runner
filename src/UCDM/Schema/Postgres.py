...
class Postgres(BaseSchema):
    ...
    def get_table_cte_column_stats(self, table_name: str, cte: str, column_name: str) -> TableStat:
        sql = "WITH {table} AS ({cte}) SELECT count(distinct \"{column}\") as unique_count from {table}".format(
            column=column_name,
            table=table_name,
            cte=cte
        )
        ...
        try:
            sql = "WITH {table} AS ({cte}) SELECT min(\"{v}\") as min_value, max(\"{v}\") as max_value, avg(\"{v}\") as avg_value from {table}".format(
                v=column_name,
                table=table_name,
                cte=cte
            )
            ...
            median50_value = self.get_median_cte(table_name, cte, column_name, min_value, max_value)
            median25_value = self.get_median_cte(table_name, cte, column_name, min_value, median50_value)
            median12_value = self.get_median_cte(table_name, cte, column_name, min_value, median25_value)
            median37_value = self.get_median_cte(table_name, cte, column_name, median25_value, median50_value)
            median75_value = self.get_median_cte(table_name, cte, column_name, median50_value, max_value)
            median63_value = self.get_median_cte(table_name, cte, column_name, median50_value, median75_value)
            median88_value = self.get_median_cte(table_name, cte, column_name, median75_value, max_value)

        except ProgrammingError as e:
            logging.debug("Can't get min/max values for {}.{} with CTE".format(table_name, column_name))
            self.engine.rollback()
        ...
        sql = "WITH {table} AS ({cte}) SELECT \"{column}\" as value, count(*) as cnt from {table} WHERE NOT \"{column}\" IS NULL GROUP BY 1 HAVING COUNT(*) >= {min_count} ORDER BY 1 DESC LIMIT 100".format(
            column=column_name,
            table=table_name,
            cte=cte,
        ...
        logging.info("Frequent values counts for {}.{} with CTE: {}".format(table_name, column_name, (values_counts)))
        nulls_count = self.fetch_row("WITH {table} AS ({cte}) SELECT COUNT(*) as cnt FROM {table} WHERE \"{column}\" is null".format(
            table=table_name,
            column=column_name,
            cte=cte
        ))['cnt']
    ...
    def get_median_cte(self, table: str, cte: str, column: str, min, max):
        if min is None or max is None:
            return None
        sql = "WITH {table} AS ({cte}) SELECT PERCENTILE_CONT(0.5) WITHIN GROUP(ORDER BY {column}) as median FROM {table} WHERE {column} > {min} and {column} < {max}".format(
            column='"' + column + '"',
            table=table,
            cte=cte,
            min=min,
